...
must_haves:
  truths:
    ...
    - "Cascade feels satisfying with escalating effects"
  artifacts:
...
export class VFXManager {
  ...
  private createParticleTextures(): void {
    ...
    if (!this.scene.textures.exists('particle_white')) {
      ...
      // Star-shaped particle (simulated with circle + larger)
      gfx.clear();
      gfx.fillStyle(0xffffff, 1);
...
Implement these effect methods:

**1. `matchPop(x, y, color)` — match clearing particle burst**
- 8-12 particles, short lifespan (300-500ms)
- Radial burst from tile center
- Use tile color as tint
- Small scale (0.3 -> 0 fade)

**2. `boosterLineSweep(startX, startY, direction, length)` — line booster activation**
- Create a trail of particles along the row/column
- Use scene.tweens to animate a light sweep from start to end
- 20-30 particles along the line
- Bright white/gold with longer trail
- This is a "wow" effect — make it dramatic with screen flash

**3. `boosterBombExplosion(x, y)` — bomb 3x3 explosion**
- 30 particles in circular burst
- Larger radius than matchPop
- Mix of white and gold particles
- Add brief camera shake: `this.scene.cameras.main.shake(150, 0.005)`
- Bright flash overlay (very brief)

**4. `boosterSphereWave(x, y)` — KLO sphere color bomb wave**
- Expanding ring effect using a tween on a circle graphic
- 40-50 particles radiating outward
- Rainbow-ish tint variety
- Camera flash: `this.scene.cameras.main.flash(200, 255, 184, 0, true)` (KLO gold flash)

**5. `confettiBurst(x, y)` — win screen confetti**
- 50 particles falling from top with gravity
- Multiple colors (KLO yellow, white, orange, blue, green)
- Longer lifespan (2-3 seconds)
- Spread across screen width
- Gravity: particles fall and drift

**6. `cascadeCombo(x, y, depth)` — escalating cascade feedback**
- Called each cascade iteration with increasing depth
- depth 1: small pop
- depth 2+: slightly bigger each time
- depth 4+: add screen shake
- Creates escalating "wow" as cascades chain

**Performance limits (hard caps for mobile):**
- matchPop: max 12 particles
- bombExplosion: max 30 particles
- sphereWave: max 50 particles
- confetti: max 50 particles
- All particles use `maxParticles` config to prevent runaway

Each method uses `this.scene.add.particles()` with `emitParticle()` or `explode()`. Particles self-clean via Phaser's built-in lifetime management.
  </action>
  <verify>
Run `npx tsc --noEmit` — no errors. VFXManager class exports correctly. All methods accept the documented parameters.
  </verify>
  <done>
VFXManager exists with 6 effect methods: matchPop, boosterLineSweep, boosterBombExplosion, boosterSphereWave, confettiBurst, cascadeCombo. Runtime particle textures generated (no external PNG dependency). Hard particle limits enforced for mobile performance.
  </done>
</task>
...
<success_criteria>
- VFXManager class with 6 particle effect methods
