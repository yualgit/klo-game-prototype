...
must_haves:
  truths:
    ...
    - "Crate obstacle (1-hit blocker) displays bubble.png"
  ...
  key_links:
    ...
    - from: "src/game/TileSprite.ts"
      to: "src/game/constants.ts"
      via: "OBSTACLE_TEXTURE_KEYS for obstacle sprite lookup"
...
4. Replace `drawObstacle()` to use PNG sprites for ALL sprite-backed obstacles (ice, dirt, AND crate/bubble):
```typescript
...
```
  ...
  <verify>
Run `npx tsc --noEmit` — no errors. Run `npm run dev` and open browser — tiles should display as PNG images (coffee cup, fuel can, wheel, light) instead of colored rectangles. Ice and grass obstacles should show their sprite images. If a crate obstacle appears, it should display bubble.png.
  </verify>
  <done>
TileSprite renders tiles using Phaser.GameObjects.Image with textures loaded from assets/tiles/. Ice obstacles display ice01-03.png progression sprites. Dirt/grass obstacles display grss01-03.png progression sprites. Crate obstacles display bubble.png (per user decision: bubble.png = 1-hit blocker). Blocked cells use programmatic fallback (no asset available). Booster overlays remain as Graphics-based overlays. Selection uses tint instead of stroke border.
  </done>
</task>
...
5. Obstacle sprites display correctly (ice/grass with damage progression, crate shows bubble.png)
