...
2. **Title "Оберіть рівень"**: Style with proper font size, slight shadow text effect.
...
- Stars below each button (keep ★/☆ text or use small gold circles).
...
<task type="auto">
  <name>Task 2: Polish Game scene — board background, win/lose overlay upgrade, HUD styling</name>
  <files>
    src/scenes/Game.ts
  </files>
  <action>
Enhance Game.ts with polished backgrounds, premium win/lose overlays, and styled HUD.
...
**3. Win overlay upgrade — animated star reveal + score counter:**
...
- Buttons use `gui_button_orange` image for primary (Далі), `gui_button_yellow` for secondary (Рівні)
...
4. Win a level — panel slides in from top, stars animate in one-by-one with elastic bounce, confetti bursts
5. Level 5 win — coupon mock displays in premium style
6. Lose a level — panel slides in, buttons use GUI sprites
7. All overlay buttons use orange/yellow PNG sprite backgrounds
8. Back button uses GUI sprite
  </verify>
  <done>
Game scene has warm gradient background with polished board area. Win overlay panel slides in with bounce, stars animate sequentially with Elastic.Out, confetti triggers on win. Lose overlay slides in with styled treatment. All buttons use GUI PNG sprite backgrounds (orange primary, yellow secondary). HUD is styled with KLO branding. Scene transitions use camera fade effects.
  </done>
</task>
...
- Obstacle sprite progression (ice, grass/dirt)
...
4. Menu has animated title, floating tiles, bounce Play button
5. LevelSelect shows road map with 5 checkpoints
6. Match clearing shows particle VFX + sound
7. Booster activation shows dramatic VFX + sound
8. Win overlay has animated star reveal + confetti
9. All buttons use GUI PNG sprite backgrounds
10. Scene transitions use camera fade
11. KLO yellow/black branding consistent throughout
12. 60fps on desktop, 30+ fps on mobile
</verification>
